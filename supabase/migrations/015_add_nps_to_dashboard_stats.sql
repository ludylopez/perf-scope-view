-- Migración: Agregar cálculo de NPS al dashboard de RRHH
-- El dashboard mostraba N/A porque la función SQL no incluía NPS, solo el fallback en TypeScript
...
DECLARE
  ...
  total_jefes INTEGER;
  total_evaluaciones_esperadas INTEGER;
...
BEGIN
  ...
  -- Total de evaluaciones esperadas = total de asignaciones activas de colaboradores
  ...
  -- Reaperturas (por ahora en 0, requiere implementar tracking)
  reaperturas := 0;

  -- Evaluaciones pendientes = total esperado - completadas - en progreso
  ...
  -- Evaluaciones por área - Contar solo colaboradores con asignación activa
  SELECT jsonb_agg(
    jsonb_build_object(
      'area', area,
  ...
  -- Evaluaciones por nivel - Contar solo colaboradores con asignación activa
  SELECT jsonb_agg(
    jsonb_build_object(
      'nivel', nivel,
      'completadas', completadas,
  ...
  SELECT jsonb_build_object(
    ...
    'npsPromedio', COALESCE(ROUND(nps_promedio, 2), NULL),
    'npsPromoters', COALESCE(nps_promoters, 0),
    'npsPassives', COALESCE(nps_passives, 0),
    'npsDetractors', COALESCE(nps_detractors, 0),
...
Incluye: evaluaciones completadas/pendientes/en progreso, promedios de desempeño/potencial,
NPS (Net Promoter Score) desde autoevaluaciones, distribución 9-box, y tendencia semanal.';
