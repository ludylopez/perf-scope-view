-- Corregir get_tendencia_semanal para usar INTERVAL correctamente
      ...
      SELECT 
        ...
        CASE 
          WHEN COALESCE(e.fecha_envio::date, DATE(e.updated_at)) IS NOT NULL THEN
            CASE 
              -- Usar INTERVAL para sumar días a la fecha
              WHEN COALESCE(e.fecha_envio::date, DATE(e.updated_at)) <= fecha_inicio_periodo + (dias_por_semana || ' days')::INTERVAL THEN 1
              WHEN COALESCE(e.fecha_envio::date, DATE(e.updated_at)) <= fecha_inicio_periodo + (dias_por_semana * 2 || ' days')::INTERVAL THEN 2
              WHEN COALESCE(e.fecha_envio::date, DATE(e.updated_at)) <= fecha_inicio_periodo + (dias_por_semana * 3 || ' days')::INTERVAL THEN 3
              WHEN COALESCE(e.fecha_envio::date, DATE(e.updated_at)) <= fecha_inicio_periodo + (dias_por_semana * 4 || ' days')::INTERVAL THEN 4
              ELSE 5
            END
          ELSE 5
        END as semana_num
