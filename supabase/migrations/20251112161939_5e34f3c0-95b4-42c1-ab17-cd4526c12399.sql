-- Migración: Corregir referencias ambiguas en funciones de dashboard
-- Problema: columnas "fecha_inicio" y "fecha_fin" causan ambigüedad con variables locales

-- 1. Corregir función get_tendencia_semanal con referencias explícitas a tablas
...
DECLARE
  ...
  semanas JSONB;
BEGIN
  -- Obtener fechas del período (convertir TIMESTAMP a DATE) - usar alias para evitar ambigüedad
  SELECT DATE(ep.fecha_inicio), DATE(ep.fecha_fin) INTO fecha_inicio_periodo, fecha_fin_periodo
      ...
      SELECT 
        ...
        CASE 
          WHEN COALESCE(e.fecha_envio::date, DATE(e.updated_at)) IS NOT NULL THEN
            CASE 
              WHEN COALESCE(e.fecha_envio::date, DATE(e.updated_at)) <= fecha_inicio_periodo + (dias_por_semana * 1) THEN 1
              WHEN COALESCE(e.fecha_envio::date, DATE(e.updated_at)) <= fecha_inicio_periodo + (dias_por_semana * 2) THEN 2
              WHEN COALESCE(e.fecha_envio::date, DATE(e.updated_at)) <= fecha_inicio_periodo + (dias_por_semana * 3) THEN 3
              WHEN COALESCE(e.fecha_envio::date, DATE(e.updated_at)) <= fecha_inicio_periodo + (dias_por_semana * 4) THEN 4
              ELSE 5
            END
          ELSE 5
        END as semana_num
...
COMMENT ON FUNCTION get_tendencia_semanal(UUID) IS 'Obtiene la tendencia semanal REAL de evaluaciones completadas basada en fechas de envío - CORREGIDA ambigüedad en fecha_inicio';