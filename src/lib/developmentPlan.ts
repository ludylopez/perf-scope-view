...
const buildEvaluationContext = (
  ...
  contexto += `- Desempeño Autoevaluación: ${resultadoFinal.desempenoAuto.toFixed(2)}/5.0\n`;
  contexto += `- Desempeño Evaluación Jefe: ${resultadoFinal.desempenoJefe.toFixed(2)}/5.0\n`;
  contexto += `- Desempeño Final (ponderado): ${resultadoFinal.desempenoFinal.toFixed(2)}/5.0\n`;
  if (resultadoFinal.potencial) {
    contexto += `- Potencial: ${resultadoFinal.potencial.toFixed(2)}/5.0\n`;
  }
  ...
  dimensions.forEach((dim, idx) => {
    ...
    contexto += `${idx + 1}. ${dim.nombre}: Auto=${autoAvg.toFixed(2)}, Jefe=${jefeAvg.toFixed(2)}, Dif=${diferencia > 0 ? '+' : ''}${diferencia.toFixed(2)}\n`;
  ...
  if (potencialDimensions && evaluacionJefe.evaluacionPotencial) {
    ...
    potencialDimensions.forEach((dim, idx) => {
      ...
      contexto += `${idx + 1}. ${dim.nombre}: ${avg.toFixed(2)}/5.0\n`;
    });
  }
...
const generateBasicDevelopmentPlan = (
  ...
  return {
    ...
    periodoId,
    competenciasDesarrollar,
    feedbackIndividual: `Su desempeño general es de ${resultadoFinal.desempenoFinal.toFixed(2)}/5.0. Se recomienda continuar desarrollando las competencias identificadas para alcanzar un nivel superior.`,
    editable: true,
    fechaCreacion: new Date().toISOString(),
  };
};
